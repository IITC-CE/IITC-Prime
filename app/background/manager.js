...
import { ajaxGet, clearWait } from "~/helpers";
...
async function versionCheck() {
  const appVersionCode = require("@nativescript/appversion");
  const currentVersion = await appVersionCode.getVersionCode();

  const lastVersion = await storage.get("lastversion").then(obj => obj["lastversion"]);
  ...
  //   }
    ...
    storage.set({ lastversion: currentVersion });
  // }
}
...
async function getUrl(url, variant, retry) {
  if (retry > 1) {
    ...
    try {
      console.warn("serverNotAvailableRetry");
...
async function downloadMeta(local, last_modified) {
  ...
  await Promise.all([p_iitc].map(fn => fn()));
}
