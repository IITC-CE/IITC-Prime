...
module.exports = env => {
  ...
  // jszip pulls in readable-stream, which references Node core modules that
  // webpack 5 no longer polyfills. We only use jszip's ArrayBuffer path, so stub
  // these out; jszip degrades its Node-stream support to false at runtime.
  config.resolve = config.resolve || {};
  config.resolve.fallback = { ...(config.resolve.fallback || {}), buffer: false, stream: false };
